-- Criando banco de dados subconsulta
CREATE DATABASE IF NOT EXISTS 
    subconsulta;

-- Criando tabela escritorios
...
FROM
    funcionarios
WHERE
    id_escritorio IN (SELECT id FROM escritorios WHERE pais = 'Brasil');
...
AND
    salario = (SELECT MAX(salario) FROM pagamentos);
...
AND
    f.id = p.id_funcionario
AND
    salario < (SELECT AVG(salario) FROM pagamentos);